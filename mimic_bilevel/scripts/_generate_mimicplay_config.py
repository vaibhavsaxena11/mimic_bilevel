...
def set_mimicplay_config(config):
    with config.experiment.values_unlocked():
        # # ensure robosuite env uses absolute pose actions
        # config.experiment.env_meta_update_dict = {"env_kwargs": {"controller_configs": {"control_delta": False}}}
        # ensure robosuite env uses delta pose actions
        config.experiment.env_meta_update_dict = {"env_kwargs": {"controller_configs": {"control_delta": True}}}

    with config.train.values_unlocked():
        # config.train.seq_length = config.algo.horizon.prediction_horizon
        config.train.seq_length = config.algo.horizon.prediction_horizon - 1
    ...
    # set noise scheduler
    # use_ddim = True
    # inf_steps = (100, 10) # (50, 5) # (train, eval)
    use_ddim = False
    inf_steps = (100, 100)

    with config.algo.values_unlocked():
        config.algo.ddim.enabled = use_ddim
        config.algo.ddpm.enabled = not use_ddim
        if len(inf_steps) > 0:
            train_inf_steps = inf_steps[0]
            eval_inf_steps = inf_steps[1]
            # set inf steps
            if use_ddim:
                config.algo.ddim.num_train_timesteps = train_inf_steps
                config.algo.ddim.num_inference_timesteps = eval_inf_steps
            else:
                config.algo.ddpm.num_train_timesteps = train_inf_steps
                config.algo.ddpm.num_inference_timesteps = eval_inf_steps
...
def modify_config_for_dataset(config, dataset_type, task_name, obs_modality, base_dataset_dir):
    ...
    with config.train.values_unlocked():
        # config.train.num_data_workers = 0  #TODO remove (added to mitigate h5py pickle error when multithreading)
        # set dataset path
        file_name = "{}.hdf5".format(task_name)
        config.train.data = [{"path": os.path.join(base_dataset_dir, dataset_type, file_name)}]
...
def generate_experiment_config(
    ...
    # set Diffusion Policy settings
    config = set_diffusion_policy_config(config=config)
    ...
    json_path = os.path.join(dir_to_save, "diffusion_policy.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_dir",
        type=str,
        default=os.path.join(mimic_bilevel.__path__[0], "exps/paper"),
    ...
    parser.add_argument(
        "--dataset_dir",
        type=str,
        default=os.path.join(mimic_bilevel.__path__[0], "../datasets"),
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=os.path.join(mimic_bilevel.__path__[0], "../training_results"),
